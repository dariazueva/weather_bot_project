...
class LogViewSetTest(APITestCase):
    def setUp(self):
        Log.objects.create(
            user_id=123, command="/weather Москва", response="Тестовый ответ"
        )
        Log.objects.create(
            user_id=13, command="/weather Санкт-Петербург", response="Тестовый ответ"
        )
        Log.objects.create(
            user_id=456, command="/weather Уфа", response="Тестовый ответ"
        )
